...
`step_measure_normalize_auc()` creates a *specification* of a recipe
step that divides each spectrum by its area under the curve (computed
using trapezoidal integration). This is useful for chromatography where
peak areas are meaningful.
...
\$\$AUC = \sum\_{i=1}^{n-1} \frac{(y_i + y\_{i+1})}{2} \cdot (x\_{i+1} -
x_i)\$\$
...
If the AUC is zero or NA, a warning is issued and the original values
are returned unchanged. At least 2 points are required for integration.
