...
`step_measure_scale_range()` creates a *specification* of a recipe step
that applies range scaling at each measurement location. This centers
and divides by the range (max - min) of each variable.
...
\$\$X\_{scaled} = \frac{X - \bar{X}}{\max(X) - \min(X)}\$\$

where \\\bar{X}\\ is the column-wise mean and the range is computed from
the training data.

If a column has zero range (constant values), that column is only
centered, not scaled.

The means and ranges are learned during
