...
Mean centering is a fundamental preprocessing step for multivariate
analysis methods like PCA and PLS. It removes the average signal at each
measurement location.

For a data matrix \\X\\ with samples as rows and measurement locations
as columns, the transformation is:

\$\$X\_{centered} = X - \bar{X}\$\$

where \\\bar{X}\\ is the column-wise mean computed from the training
data.

The means are learned during
...
this step after training, a tibble with the learned means at each
