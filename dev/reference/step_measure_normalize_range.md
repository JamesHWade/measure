...
`step_measure_normalize_range()` creates a *specification* of a recipe
step that applies min-max normalization to scale each spectrum to the
range 0 to 1.
...
\$\$x\_{norm} = \frac{x - \min(x)}{\max(x) - \min(x)}\$\$

After transformation, the minimum value of each spectrum will be 0 and
the maximum will be 1.

If the range is zero (constant spectrum), a warning is issued and
centered values are returned (minimum subtracted but no scaling).
