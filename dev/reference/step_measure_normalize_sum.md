...
`step_measure_normalize_sum()` creates a *specification* of a recipe
step that divides each spectrum by its sum (total intensity). This is
useful for comparing relative abundances across samples with different
total signals.
...
\$\$x\_{norm} = \frac{x}{\sum x}\$\$

After transformation, the sum of each spectrum will equal 1.

If the sum is zero or NA, a warning is issued and the original values
