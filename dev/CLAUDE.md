...
`measure` is an R package that extends tidymodels with preprocessing
steps for analytical measurement data (spectroscopy, chromatography,
mass spectrometry). It provides a recipes-style interface for spectral
...
Key helper functions in `R/helpers.R`: - `measure_to_matrix()` /
`matrix_to_measure()`: Convert between formats -
[`find_measure_cols()`](https://jameshwade.github.io/measure/dev/reference/find_measure_cols.md):
Detect measure columns by class -
[`check_for_measure()`](https://jameshwade.github.io/measure/dev/reference/check_for_measure.md)
/ `check_has_measure()`: Validation utilities

### Tunable Parameters

Parameters for hyperparameter tuning with
[`tune::tune()`](https://hardhat.tidymodels.org/reference/tune.html) are
defined in `R/parameters.R`. Each tunable step parameter has a
corresponding `dials` function (e.g.,
...
2.  **Processing**: Chain of `step_measure_*()` functions apply
    transformations
3.  **Output**:
...
Steps are grouped by functionality: - `input_*.R` / `output_*.R`: Data
format conversion - `baseline-*.R`: Baseline correction methods (ALS,
polynomial, etc.) - `smooth*.R`: Smoothing/filtering operations -
`align.R`: Spectrum alignment (DTW, PTW, COW) - `scale.R` /
`normalize.R`: Variable-wise scaling / sample-wise normalization -
`peak-operations.R`: Peak detection and integration - `qc.R`: Quality
control steps - `axis-validation.R`: Data validation utilities -
...
creates calibration curves with weighted regression -
...
calculate detection/quantitation limits
...
builds ISO GUM-compliant uncertainty budgets -
...
evaluates results against criteria - Preset criteria:
...
generates HTML/PDF/Word using Quarto templates - Templates in
`inst/templates/`: ICH Q2(R2) and USP \<1225\> formats

## Testing

Tests use testthat edition 3 with snapshots in `tests/testthat/_snaps/`.
Helper data is in `tests/testthat/helpers-*.R`.

## Python Integration

Some baseline methods use `pybaselines` via reticulate. The Python
dependency is configured in DESCRIPTION under `Config/reticulate`.
...
- S3 methods for validation objects should include
...
The package includes a registry system for technique packs (external
packages that extend measure):
...
- **[`register_measure_pack()`](https://jameshwade.github.io/measure/dev/reference/register_measure_pack.md)**:
  For extension packages to register themselves
- **[`register_measure_step()`](https://jameshwade.github.io/measure/dev/reference/register_measure_step.md)**:
  For registering individual steps
- **[`measure_packs()`](https://jameshwade.github.io/measure/dev/reference/measure_packs.md)**:
  List all registered technique packs
- **[`measure_steps()`](https://jameshwade.github.io/measure/dev/reference/measure_steps.md)**:
  List all registered steps (core + extensions)

Steps marked as superseded (e.g., SEC/GPC steps) point users to the
appropriate technique pack (e.g., `measure.sec`).

## Dependencies

- Packages in `Suggests` must be available on CRAN or Bioconductor
- Technique packs like `measure.sec` that aren’t published yet should
  NOT be in Suggests
