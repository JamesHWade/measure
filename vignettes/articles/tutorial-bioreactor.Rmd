---
title: "Tutorial: Bioreactor Glucose Prediction"
---
...
## Introduction
This vignette demonstrates a complete chemometric modeling workflow using measure. We'll predict glucose concentration in bioreactors using Raman spectroscopy data.

## The dataset

The `glucose_bioreactors` dataset contains Raman spectra from bioreactor experiments described in Kuhn and Johnson's *Feature Engineering and Selection* (2020).
...
Each dataset has:
- 2,651 spectral channels (Raman wavenumbers)
- `glucose`: the target variable to predict
- `day`: sampling day (1-14)
...
The preprocessed data has fewer columns because:
- The `day` and `batch_id` columns are preserved as ID columns
- The 500 spectral columns have been transformed and renamed with prefix `raman_`
...
The key advantage of the recipes framework is consistent preprocessing of new data:
...
- The recipe framework ensures identical preprocessing for training and test data
- `step_measure_input_wide()` handles the conversion of many spectral columns
- Processing steps like Savitzky-Golay and SNV remove unwanted variation
- `step_measure_output_wide()` produces modeling-ready data
...
- [Getting Started](tutorial-getting-started.html) - Fundamentals of measure
- [Integrating with tidymodels](howto-tidymodels.html) - Advanced modeling workflows with tuning
- [Preprocessing Reference](reference-preprocessing.html) - Complete guide to preprocessing techniques
