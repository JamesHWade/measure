...
measure provides several built-in preprocessing steps (`step_measure_savitzky_golay()`, `step_measure_snv()`, `step_measure_msc()`), plus `step_measure_map()` for custom transformations when the built-in steps don't cover your needs. See `vignette("preprocessing")` for details on all available options.
...
- See `vignette("preprocessing")` for a detailed guide to each preprocessing technique
- Explore hyperparameter tuning with `tune` - measure's Savitzky-Golay step is tunable!
